...
public class UserFolderDAO extends BaseDAO {
    ...
    private FolderTO toFolderTO(Row row) {
        ...
        if (row != null) {
            ...
            folderTO.setFolderId(row.get(Folder.FOLDERID).asString().get());
    ...
    String getFolderId(String userId, String folderName) {
        ...
        /* index is on userId and foldername*/
        IndexKey key = index.createIndexKey();
        key.put(User.USERID, userId);

        // get all the rows that have userId and folderName
        ...
        if (results != null && results.hasNext()) {
            while (results.hasNext()) {
                row = results.next();
                folderTO = this.toFolderTO(row);
                // System.out.println("folder row "+folderTO.getName()+" folder search "+folderName);
                if (folderTO.getName().equals(folderName)) {
                    folderId = folderTO.getFolderId();
                }
            }

        } // if(results.hasNext())
        // System.out.println("folder id: "+folderTO.getFolderId());
        return folderId;
    }
